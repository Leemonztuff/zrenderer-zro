...
const ROSpriteBillboard = ({ baseUrl, accessToken, spriteParams, scale = 0.02, ...props }) => {
    ...
    const renderUrl = useMemo(() => {
        // Asegurar que job sea array de strings
        const jobs = Array.isArray(spriteParams.job) ? spriteParams.job.map(String) : [String(spriteParams.job)];
        const params = { ...spriteParams, job: jobs };

        return `${baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl}/render?downloadimage=true`;
    }, [baseUrl, JSON.stringify(spriteParams)]);
    ...
    // Manejar la limpieza de la textura de forma separada para evitar fugas de memoria GPU
    useEffect(() => {
        return () => {
            if (texture) {
                texture.dispose();
